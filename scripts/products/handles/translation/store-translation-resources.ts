...
const ALLOWED_KEYS = new Set(["handle"]);

console.log("Fetching all PRODUCT translatable resources (handles only)...");
...
try {
  do {
    ...
    console.log(
      `Page ${page}: fetched ${nodes.length} resources (${allResources.length} total with handle)`
    );
  ...
  const outputPath = new URL(
    "./product-handle-translation-resources.json",
    import.meta.url
  );
